...
struct ES_MatchingSchema_Ensamble
{
	unsigned* sigma1;
	size_t sigma1l;
	...
	ES_MatchingSchema_Ensamble() :
			costValue(0.0), sigma1l(0), sigma2l(
			0), sigma1(NULL), sigma2(NULL)
	{

	}

    ES_MatchingSchema_Ensamble(const std::vector<unsigned>&_sigma_1,
					  const std::vector<unsigned>&_sigma_2) :
			costValue(0.0), sigma1l(
			_sigma_1.size()), sigma2l(_sigma_2.size())
	{
		sigma1 = new unsigned[_sigma_1.size()];
		std::iota(sigma1, sigma1 + _sigma_1.size(), 0);
    ...
    ES_MatchingSchema_Ensamble(const ES_MatchingSchema_Ensamble& m) :
			sigma1l(m.sigma1l), sigma2l(m.sigma2l), costValue(m.costValue)
	{
		sigma1 = new unsigned[sigma1l];
		std::copy(m.sigma1, m.sigma1 + sigma1l, sigma1);
...
//			std::cout << "SECOND VECTOR: i_1=" << i_1 << " i_2=" << i_2 << "\n";
	...
	bool operator<(const ES_MatchingSchema_Ensamble& m) const
	{
		return this->costValue > m.costValue;
	}

    ES_MatchingSchema_Ensamble& operator=(const ES_MatchingSchema_Ensamble& m)
	{
		costValue = m.costValue;
...
struct ES_MatchingSchema_EnsambleEdit
{
	unsigned* sigma1;
	size_t sigma1l;
	...
	ES_MatchingSchema_EnsambleEdit() :
			costValue(std::numeric_limits<unsigned int>::max()), sigma1l(0), sigma2l(
			0), sigma1(NULL), sigma2(NULL)
	{
	...
	ES_MatchingSchema_EnsambleEdit(const std::vector<unsigned>&_sigma_1,
							   const std::vector<unsigned>&_sigma_2) :
			costValue(std::numeric_limits<double>::max()), sigma1l(
			_sigma_1.size()), sigma2l(_sigma_2.size())
	{
		sigma1 = new unsigned[_sigma_1.size()];
		std::iota(sigma1, sigma1 + _sigma_1.size(), 0);
	...
	ES_MatchingSchema_EnsambleEdit(const ES_MatchingSchema_EnsambleEdit& m) :
			sigma1l(m.sigma1l), sigma2l(m.sigma2l), costValue(m.costValue)
	{
		sigma1 = new unsigned[sigma1l];
		std::copy(m.sigma1, m.sigma1 + sigma1l, sigma1);
...
//			std::cout << "SECOND VECTOR: i_1=" << i_1 << " i_2=" << i_2 << "\n";
	...
	bool operator<(const ES_MatchingSchema_EnsambleEdit& m) const
	{
		return this->costValue < m.costValue;
	}

	ES_MatchingSchema_EnsambleEdit& operator=(const ES_MatchingSchema_EnsambleEdit& m)
	{
		costValue = m.costValue;
...
#endif //MPED_ES_MATCHINGSCHEMA_H
